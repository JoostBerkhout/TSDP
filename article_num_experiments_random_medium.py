...
for random_seed in range(numb_exp):
    ...
    DeltaSol = goal_MC_optimization(
        ...
        verbose=False, timeLimit=timeLimit
        )
    run_time = time.time() - start_time
    store_results(
        results_dict, method_name, P, DeltaSol, mu_goal, normType,
    ...
    DeltaSol = R1SH(
        ...
        jumpDirectlyToGoalIfPossible=False,
        considerDeltaHistory=True,
        trackBestSolutionAlongTheWay=False,
        timeLimit=timeLimit
        )
    run_time = time.time() - start_time
    store_results(
    ...
    print(f'Results after seed {random_seed} is done:')
    results_table = [[method] + [np.round(np.nanmean(results[k]), 4)
                                 for k, v in results.items()]
                     for method, results in results_dict.items()]
    column_labels = ['Method', f'mean(\|Delta\|_{normType})',
                     'Fraction feasible', 'mean run time', 'mean rank']
