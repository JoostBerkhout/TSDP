...
Author: Joost Berkhout (VU, email: joostberkhout@vu.nl)

Description: This script is used to do experiments for TSDP_optimization.py
regarding the sparse email network.
...
G = np.genfromtxt('Data\\Prepared data\\email_P.csv', delimiter=',')
mu = calc_stationary_distribution(G)
n = len(mu)

# find cliques in order from large to small
cliques = find_cliques_from_adjacency_matrix(G)
...
for idx, clique in enumerate(cliques_to_consider):
    ...
    DeltaSol = min_norm_rank_1(G, mu_goal, normType)
    run_time = time.time() - start_time
    store_results(
        results_dict, method_name, G, DeltaSol, mu_goal, normType,
        run_time, calc_norm=True
        )

    method_name = 'min_norm_rank_1_pert_pres_stoch()'
    start_time = time.time()
    alpha, soft_mu_goal = max_convex_softened_mu_goal(G, mu, mu_goal)
    if alpha > 0:
    ...
    start_time = time.time()
    DeltaSol = DeltaSolErgProj = ergodic_project(mu_goal) - G
    run_time = time.time() - start_time
    store_results(
        results_dict, method_name, G, DeltaSol, mu_goal, normType,
        run_time
